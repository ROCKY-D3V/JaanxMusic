...
async def mentionall(client, message):
    ...
    if message.chat.type == ChatType.PRIVATE:
        return await message.reply("тнιѕ ¢σммαи∂ σиℓу fσя gяσυρѕ.")
    ...
    try:
        ...
        if participant.status in (
            ...
            is_admin = True
    if not is_admin:
        return await message.reply("уσυ αяє иσт α∂мιи вαву, σиℓу α∂мιиѕ ¢αи . ")

    if message.reply_to_message and message.text:
        return await message.reply("/shayaril  туρє ℓιкє тнιѕ / яєρℓу αиу мєѕѕαgє иєχт тιмє ")
    elif message.text:
        mode = "text_on_cmd"
        msg = message.text
        ...
        if not msg:
            return await message.reply("/shayari  туρє ℓιкє тнιѕ / яєρℓу αиу мєѕѕαgє иєχт тιмє ...")
    else:
        return await message.reply("/shayari  туρє ℓιкє 𝐓𝐡𝐢𝐬 / яєρℓу αиу мєѕѕαgє иєχт тιмє ..")
    if chat_id in spam_chats:
        return await message.reply("ρℓєαѕє αт fιяѕт ѕтσρ яυииιиg ρяσ¢єѕѕ ...")
...
async def cancel_spam(client, message):
    if not message.chat.id in spam_chats:
        return await message.reply("αвнι мαι инι нυи ..")
    is_admin = False
    try:
        ...
        if participant.status in (
            ...
            is_admin = True
    if not is_admin:
        return await message.reply("тυм α∂мιи инι нσ ѕιяf α∂мιи, тαg кαя ѕαктα нαι мємвєяѕ кσ.")
    else:
        try:
            spam_chats.remove(message.chat.id)
        except:
            pass
        return await message.reply("😜вαи∂ вσ gуα😜")
